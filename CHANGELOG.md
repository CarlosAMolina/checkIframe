...
- By default, internal debug logging is disabled (these logs are different from the logs generated by content script). It can be enabled manually by changing the `debug` variable in `src/logger.js`.
